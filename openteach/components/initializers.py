...
from .recorders.oak_image import RGBImageRecorder, DepthImageRecorder
...
class ProcessInstantiator(ABC):
    ...
    def _start_component(self,configs):
        import os
        print(f"[START_COMPONENT] pid={os.getpid()} configs={configs}")
...
# Teleop whill flag should be TRUE
...
# Collector whill flag should be FALSE
...
class Collector(ProcessInstantiator):
    ...
    def _init_camera_recorders(self):
        ...
        if self.configs.sim_env is not True:
            for cam_idx in range(len(self.configs.robot_cam_serial_numbers)):
                self.processes.append(Process(target=self._start_rgb_component, args=(cam_idx,))) #cam_0
        ...
        if ros_arm_cam_number is not None:
            self.processes.append(Process(target=self._start_ros_arm_rgb, args=(ros_arm_cam_number,))) #cam_2
